...
void RegisterStdStringUtils( asIScriptEngine *engine )
{
	...
	r = engine->RegisterObjectMethod( XorStr( "string" ) , XorStr( "array<string>@ split(const string &in) const" ) , asFUNCTION( StringSplit_Generic ) , asCALL_GENERIC ); assert( r >= 0 );
	r = engine->RegisterGlobalFunction( XorStr( "string join(const array<string> &in, const string &in)" ) , asFUNCTION( StringJoin_Generic ) , asCALL_GENERIC ); assert( r >= 0 );
}
