...
namespace source
{
	...
	{		
		...
		auto ScriptModule::PrepateFunctionContext( asIScriptContext* script_context , asIScriptModule* script_module , const char* function_decl ) -> asIScriptFunction*
		{
			asIScriptFunction* script_function = script_module->GetFunctionByDecl( function_decl );

			if ( script_function )
			{
				script_context->ClearLineCallback();
				script_context->SetLineCallback( asFUNCTION( LineCallback ) , &m_timeout , asCALL_CDECL );

				if ( script_context->GetState() == asEXECUTION_ACTIVE )
					return nullptr;
		
				script_context->Prepare( script_function );

				m_timeout = GetTickCount() + EXECUTE_TIMEOUT;
		...
		auto ScriptModule::ExecuteScriptContext( asIScriptContext* script_context , asIScriptModule* script_module ) -> bool
		{
			auto& notify = feature::Notification::Instance();
		...
		{
			if ( !m_script_context || !m_script_module )
				return false;

			auto script_function_init = PrepateFunctionContext( m_script_context , m_script_module , XorStr( "void Init()" ) );

			if ( script_function_init )
				return ExecuteScriptContext( m_script_context , m_script_module );
		...
		auto ScriptModule::CallRender() -> bool
		{
			...
			auto script_function_init = PrepateFunctionContext( m_script_context , m_script_module , XorStr( "void Render()" ) );
		...
		auto ScriptModule::CallFireGameEvent( IGameEvent* pEvent ) -> bool
		{
			...
			auto script_function_init = PrepateFunctionContext( m_script_context , m_script_module , XorStr( "void FireGameEvent(IGameEvent@)" ) );
		...
		auto source::engine::ScriptModule::CallCreateMove( float flInputSampleTime , CUserCmd* pCmd ) -> bool
		{
			...
			auto script_function_init = PrepateFunctionContext( m_script_context , m_script_module , XorStr( "void CreateMove(float,CUserCmd@)" ) );
			...
			if ( script_function_init )
			{
				...
				ExecuteScriptContext( m_script_context , m_script_module );
			}
		...
		auto ScriptSystem::Destroy() -> void
		{
			...
			{
				module.m_script_context->Abort();
				module.m_script_context->Release();
			}
		...
		auto ScriptSystem::BuildModule( string file_name , string module_name ) -> ScriptModule*
		{
			...
			if ( r < 0 )
			{
				notify.AddNotification( 10 , feature::nt_error , XorStr( "#1 (%s) Unrecoverable error while starting a new module." ) , module_name.c_str() );
			...
			if ( r < 0 )
			{
				notify.AddNotification( 10 , feature::nt_error , XorStr( "#2 (%s) Please correct the errors in the script and try again." ) , module_name.c_str() );
			...
			if ( r < 0 )
			{
				notify.AddNotification( 10 , feature::nt_error , XorStr( "#4 (%s) Please correct the errors in the script and try again." ) , module_name.c_str() );
				//Andromeda::WriteDebugLog( XorStr( "(%s) Please correct the errors in the script and try again.\n" ) , module_name.c_str() );
				return nullptr;
			}
			...
			if ( !script_module )
			{
				notify.AddNotification( 10 , feature::nt_error , XorStr( "#4 (%s) Can't find module" ) , module_name.c_str() );
